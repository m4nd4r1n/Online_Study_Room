...
public interface StaticsRepository extends JpaRepository<StudyTime,Long> {
    ...
    @Query(value = "select sec_to_time(sum(time_to_sec(s.endTime)-time_to_sec(s.startTime))) as time " +
            "from Studytime s join User u on s.mteId = u.email join Mentee m on m.mteId = s.mteId join Parent p on p.prtId = m.prtid ",nativeQuery = true)
    public List<StaticsInterface> findByWeekStudyTime(String PtrId, LocalDateTime date);
